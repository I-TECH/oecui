...
final class RequestDispatchingThread extends Thread {
    ...
    /**
     * This constructor is used when a synchronous server response is expected
     * for the request to be dispatched.
     *
     ...
     * @param requestTypeId {@link RequestTypeId} of the request to be
     * dispatched
    ...
    RequestDispatchingThread(Mediator mediator, int requestTypeId,
            Object requestData) {
        this(mediator, requestTypeId, requestData, null);
    }

    /**
     * Asynchronously forwards a request to a server. If
     * {@link RequestDispatchingThread#requestResult} is not null, a response
     * will be awaited. Otherwise the method will return immediately.
     */
    @Override
    public void run() {
        if (wait) {
